const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4940';
...
const adminCookie = await login('admin@space-mission-operations.local', 'admin123');
const managerCookie = await login('manager@space-mission-operations.local', 'manager123');
const analystCookie = await login('analyst@space-mission-operations.local', 'analyst123');
